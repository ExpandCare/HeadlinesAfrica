...
Parse.Cloud.define("searchPost", function(request, response) 
{
   ...
   contentQuery.find(
   {
     success: function(results) 
     {
       ...
       {
          var posts = [];
          console.log('results count = '.concat(results.length));
          for(var i = 0; i < results.length; i++)
          {
           ...
           responseDict["count"]   = results.length;
     ...
     error: function(error) 
     {
      console.log('ERROR CODE = '.concat(error.code));
        if(error.code == 154)
        {
...
function tokensFromString(contentString) 
{
   ...
   console.log('TOKENS = '.concat(tokens));
   return tokens;
}
